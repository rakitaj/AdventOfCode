import pytest
from src.aoc2024.day07 import parse_line, calculator, calculator_backwards_with_concat
...
def test_calculate_add_and_mult_known_good(line: str):
    ...
    assert calculator(target, numbers) is True
...
def test_calculate_add_and_mult_known_bad(line: str):
    target, numbers = parse_line(line)
    assert calculator(target, numbers) is False
...
def test_calculate_add_and_mult_known_good_with_concat(line: str):
    target, numbers = parse_line(line)
    assert calculator_backwards_with_concat(target, numbers) is True
...
def test_calculate_add_and_mult_known_bad_with_concat(line: str):
    target, numbers = parse_line(line)
    assert calculator_backwards_with_concat(target, numbers) is False


def test_calculate_with_concat_degenerate_case():
    line = "1517: 7 1 9 701 797"
    target, numbers = parse_line(line)
    assert calculator_backwards_with_concat(target, numbers) is False
